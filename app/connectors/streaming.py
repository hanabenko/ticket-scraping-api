...
def load_streams_file(path: str) -> Iterable[Dict[str, Any]]:
    ...
    for row in data:
        yield {
            ...
            "channel": row.get("channel", "streaming"),
            "occurred_at": row.get("occurred_at"),
            "metadata": {k: v for k, v in row.items() if k not in {"artist","email","occurred_at","channel"}},
        }
...
def load_streams_http(url: str):
    ...
    for row in payload:
        yield {
            ...
            email: row.get(email),
            interaction_type: (row.get(type) or row.get(interaction_type) or (stream if streaming==streaming else click)),
            channel: row.get(channel) or streaming,
