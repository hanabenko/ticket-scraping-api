...
def load_merch_file(path: str) -> Iterable[Dict[str, Any]]:
    data = read_json_file(path)
    for row in data:
        yield {
            "artist_name": row.get("artist"),
            "email": row.get("email"),
            "interaction_type": "merch_purchase" if row.get("purchased", False) else "click",
            "channel": row.get("channel", "merch"),
            "occurred_at": row.get("occurred_at"),
            "metadata": {k: v for k, v in row.items() if k not in {"artist","email","occurred_at","channel","purchased"}},
        }
...
def load_merch_http(url: str):
    ...
    for row in payload:
        yield {
            ...
            email: row.get(email),
            interaction_type: (row.get(type) or row.get(interaction_type) or (stream if merch==streaming else click)),
            channel: row.get(channel) or merch,
